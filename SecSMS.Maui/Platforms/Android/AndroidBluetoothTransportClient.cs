...
{
    const int MaxPayloadSize = 65536; // 64KB max payload to prevent OOM attacks
    ...
    {
        TryExecute(() => _receiveCts?.Cancel());

        _receiveCts?.Dispose();
        _receiveCts = null;

        TryExecute(() => _input?.Dispose());
        TryExecute(() => _output?.Dispose());
        TryExecute(() => _socket?.Close());
    ...
    async Task ReceiveLoopAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    throw new InvalidDataException($"Payload length {length} exceeds maximum allowed size of {MaxPayloadSize}.");
                }
